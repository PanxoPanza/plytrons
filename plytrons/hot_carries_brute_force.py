...
def _hot_e_dist_parallel(
    ...
    Mfi_all = np.zeros((N, N), dtype=np.float64)
    ...
    for lf in prange(lmax):
        ...
        for li in range(lmax):
            ...
            Mfi_block = np.zeros((Ef_len, Ei_len), dtype=np.float64)
            # Mif_block = np.zeros((Ei_len, Ef_len), dtype=np.float64)
            ...
            for mf in range(-lf, lf + 1):
                for mi in range(-li, li + 1):
                    ...
                    Mfi = _transition_M(sf, si, a_nm, X_lm, state_lf, state_li)
                    # Mif = _transition_M(si, sf, a_nm, X_lm, state_li, state_lf)
                    Mfi_block += (Mfi.real**2 + Mfi.imag**2)
                    # Mif_block += (Mif.real**2 + Mif.imag**2)
    ...
    TTe = 4.0 / tau_e_fs * fd_i * (1.0 - fd_f) * (
        Mfi_all / denom_e + Mfi_all.T.conj() / denom_h
    )
    TTh = 4.0 / tau_e_fs * fd_f * (1.0 - fd_i) * (
        Mfi_all / denom_h + Mfi_all.T.conj() / denom_e
    )
